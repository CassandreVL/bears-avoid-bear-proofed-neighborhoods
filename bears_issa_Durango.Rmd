---
title: "Bears decreased their use of residential neighborhoods after garbage availability was reduced. R Code"
author: "Cassandre Venumière-Lefebvre"
date: "`r Sys.Date()`"
...
#parcels that are either parks or cut by the experimental zone delineation
#parcels_to_remove = data.frame(Id = c(1,2,3,4,5,7,8,185,6,3,5,196,197,198,195,253,707,709,710,713,725,727,728,726,715,748,753,779,767,768,787,782,783,786,889,888,1181,1184,1185,1186,1239,1240,1241,790,883,881,887,886,791,795,794,792,793,865,948,1028,951,952,1033,1032,1035,1034,1137,1029,1828,2051,2052,2053,2054,2055,1880,1882,1879,1878,1881,2731,2730,2989,2970,2517,2721,2720,2318,947,946,949,1030,1827,1575,1574,2134,2179,2180,2185,2184,2181,2182,2183,2288,2289,2290,2291,2285,2286,2287,2359,2360,2969,261,260,654,402,468,863,829,641,920,1143,1475,1690,1756,2027,1874,1829,1928,2852))
#parcels = anti_join(parcels, parcels_to_remove)
...
#out = st_union(districts) %>% st_difference(buffer,.) %>% st_sf %>% mutate(area = as.numeric(st_area(.))) %>% terra::rasterize(., nlcd, field = "area", background = NA, touches = F) %>% crop(frame)
#parcel_area = districts %>% mutate(area = as.numeric(st_area(districts))) %>% terra::rasterize(., nlcd, field = "area", background = NA, touches = T) %>% crop(frame)
#writeRaster(parcel_area, filename = "data/parcel_area.tif", overwrite = T)
#new_parcel_area = merge(parcel_area, out)
#writeRaster(new_parcel_area, filename = "data/new_parcel_area.tif", overwrite = T)
...
#write.csv(cor.table, "output/cortable.csv")
```
...
register_google(key = "AIzaSyAg64QUvN8kvqnV_ioD9Hr3angUpV7wmCw")
...
```{r movement effects 1 treatment}
#Speed
